...
var OneAtabiInWei sdk.Int = sdk.NewInt(1_000_000_000_000)
...
func (k BaseSendKeeper) SubWei(ctx sdk.Context, addr sdk.AccAddress, amt sdk.Int) (err error) {
	...
	if amt.LTE(currentWeiBalance) {
		// no need to change atabi balance
		return k.setWeiBalance(ctx, addr, currentWeiBalance.Sub(amt))
	}
	currentAtabiBalance := k.GetBalance(ctx, addr, sdk.MustGetBaseDenom()).Amount
	currentAggregatedBalance := currentAtabiBalance.Mul(OneAtabiInWei).Add(currentWeiBalance)
	...
	atabiBalance, weiBalance := SplitAtabiWeiAmount(postAggregatedbalance)
	if err := k.setBalance(ctx, addr, sdk.NewCoin(sdk.MustGetBaseDenom(), atabiBalance), true); err != nil {
		return err
	}
...
func (k BaseSendKeeper) AddWei(ctx sdk.Context, addr sdk.AccAddress, amt sdk.Int) (err error) {
	...
	if postWeiBalance.LT(OneAtabiInWei) {
		// no need to change atabi balance
		return k.setWeiBalance(ctx, addr, postWeiBalance)
	}
	currentAtabiBalance := k.GetBalance(ctx, addr, sdk.MustGetBaseDenom()).Amount
	atabiCredit, weiBalance := SplitAtabiWeiAmount(postWeiBalance)
	if err := k.setBalance(ctx, addr, sdk.NewCoin(sdk.MustGetBaseDenom(), currentAtabiBalance.Add(atabiCredit)), true); err != nil {
		return err
	}
	return k.setWeiBalance(ctx, addr, weiBalance)
}
...
func SplitAtabiWeiAmount(amt sdk.Int) (sdk.Int, sdk.Int) {
	return amt.Quo(OneAtabiInWei), amt.Mod(OneAtabiInWei)
}
